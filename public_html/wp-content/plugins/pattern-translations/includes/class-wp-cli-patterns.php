<?php
/**
 * Pattern translation CLI commands.
 *
...
class WP_CLI_Patterns extends WP_CLI_Command {
	 ...
	 *
	 * @param array $_    Positional CLI arguments.
	 * @param array $args Named CLI arguments.
	 * @return void
	 *
	...
	public function json( $_, $args ) {
		...
		WP_CLI::log( wp_json_encode( $patterns, JSON_PRETTY_PRINT ) );
	}
	...
	/**
	 * Select patterns using the shared CLI flags.
	 *
	 * @param array $args Named CLI arguments.
	 * @return Pattern[] Selected patterns.
	 */
	private function get_patterns_or_exit( $args ) {
		...
		if ( empty( $patterns ) ) {
			WP_CLI::error( 'No patterns found for args: ' . wp_json_encode( $args ) . ', query: ' . wp_json_encode( $query ) );
			exit( 1 );
		}
